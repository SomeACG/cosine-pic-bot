...
export const processArtworks = async (
  ...
  try {
    ...
    const commonOpts = { ctx, customTags, option, totalPage };
    ...
    if (!batch) {
      if (!page) {
        ...
        for (let page = 0; page < totalPage; page++) {
          ...
          await echoPostMedia({ ...commonOpts, artworks: artworks ?? [], currentPage: page });
        }
        ...
        await echoPostMedia({ ...commonOpts, artworks });
      ...
      if (!page) {
        ...
        await echoPostMedia({ ...commonOpts, artworks: target });
